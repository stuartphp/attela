...
<div class="card shadow mb-4">
    <div class="card-header">
        <div class="row">
            <div class="col-md-6">{{ __('global.menu.accounting.title') }} / <a href="{{ route('assets.index') }}">{{ __('assets.title') }}</a> </div>
            <div class="col-md-6 text-right">
                @if(count(array_intersect(session()->get('grant'), ['SU', 'assets_create']))==1)
                <a href="#"><i id="create_record" class="bi bi-plus fa-1x"></i></a>
                @endif
                <form class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search" action="{{ route('assets.index') }}" method="get">
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table class="table table-hover" id="dataTable" width="100%" cellspacing="0">
          <thead>
            <tr>
               <th>{{__('assets.fields.asset_number')}}</th>
               <th>{{__('assets.fields.asset_type_id')}}</th>
               <th>{{__('assets.fields.employee_id')}}</th>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="form-group row">
                <label class="col-md-3">{{__('assets.fields.asset_number')}}</label>
                <div class="col-md-9">
                    <input type="text" name="asset_number" id="asset_number" class="form-control">
                </div>
            </div><div class="form-group row">
                <label class="col-md-3">{{__('assets.fields.asset_type_id')}}</label>
                <div class="col-md-9">
                    <input type="number" name="asset_type_id" id="asset_type_id" class="form-control">
                </div>
            </div><div class="form-group row">
                <label class="col-md-3">{{__('assets.fields.employee_id')}}</label>
                <div class="col-md-9">
                    <input type="number" name="employee_id" id="employee_id" class="form-control">
                </div>
            </div><div class="form-group row">
                <label class="col-md-3">{{__('assets.fields.serial_number')}}</label>
                <div class="col-md-9">
                    <input type="text" name="serial_number" id="serial_number" class="form-control">
                </div>
            </div><div class="form-group row">
                <label class="col-md-3">{{__('assets.fields.description')}}</label>
                <div class="col-md-9">
...
$(document).on('change', '.action', function(){
    ...
    switch(val)
    {
        case 'Edit':
        $.ajax({
                url:'/accounting/assets/'+id+'/edit',
                dataType:'JSON',
                method: 'GET',
                success: function (response) {
                    $('.modal-title').html(update);
                    $('#action').val(update);
                    $('#main_form').attr('action', "/accounting/assets/"+id);
                    $('#method').val('PUT');
                    $('#id').val(response.data.id);
					$('#company_id').val(response.data.company_id);
					$('#asset_number').val(response.data.asset_number);
					$('#asset_type_id').val(response.data.asset_type_id);
