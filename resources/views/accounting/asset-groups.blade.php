...
<div class="card shadow mb-4">
    <div class="card-header">
        <div class="row">
            <div class="col-md-6">{{ __('global.menu.accounting.title') }} / <a href="{{ route('asset-groups.index') }}">{{ __('asset_groups.title') }}</a> </div>
            <div class="col-md-6 text-right">
                @if(count(array_intersect(session()->get('grant'), ['SU', 'asset_groups_create']))==1)
                <a href="#"><i id="create_record" class="bi bi-plus fa-1x"></i></a>
                @endif
                <form class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search" action="{{ route('asset-groups.index') }}" method="get">
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table class="table table-hover" id="dataTable" width="100%" cellspacing="0">
          <thead>
            <tr>
               <th>{{__('asset_groups.fields.name')}}</th>
...
<tr>
    ...
    @endforeach
    @else
<tr><td colspan="2">{{ __('global.no_results') }}</td></tr>
    @endif
</tbody>
        </table>
    </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="form-group row">
                <label class="col-md-3">{{__('asset_groups.fields.name')}}</label>
                <div class="col-md-9">
...
$(document).on('change', '.action', function(){
    ...
    switch(val)
    {
        case 'Edit':
        $.ajax({
                url:'/accounting/asset-groups/'+id+'/edit',
                dataType:'JSON',
                method: 'GET',
                success: function (response) {
                    ...
                    $('#main_form').attr('action', "/accounting/asset-groups/"+id);
                    $('#method').val('PUT');
        ...
        case 'Delete':
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        url: '/accounting/asset-groups/' + id,
                        dataType: 'JSON',
