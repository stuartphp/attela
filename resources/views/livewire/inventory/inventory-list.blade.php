<div>
    <div class="head mb-1 ms-2 me-2">
        ...
        <div class="row gx-2 align-ite-s-center">
            <div class="col-sm-2">
                <input  wire:model.lazy="size" type="text" class="form-control form-control-sm"/>
            </div>
            <div class="col-sm-10">
                <input wire:model.lazy="search" type="text" class="form-control form-control-sm mb-2" placeholder="{{ __('global.search') }}" aria-label="Search" aria-describedby="basic-addon2">
            </div>
    ...
    <div class="list-group ms-3 me-3">
        ...
        <button type="button" onclick="getDetail({{ $item->id}})" class="list-group-item list-group-item-action @if($item->is_active==0) list-group-item-danger @else  list-group-item-success @endif" id="I{{ $item->id}}">{{ strlen($item->item_code.' '.$item->description)>48 ? substr($item->item_code.' '.$item->description,0,46).'...' : $item->item_code.': '.$item->description }}</button>
        @endforeach
