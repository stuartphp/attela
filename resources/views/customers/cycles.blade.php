...
<div class="card shadow mb-4">
    <div class="card-header">
        <div class="row">
            <div class="col-md-6">{{ __('customers.title') }} / <a href="{{ route('cycles.index') }}">{{ __('customer_cycles.title') }}</a> </div>
            <div class="col-md-6 text-right">
                @if(count(array_intersect(session()->get('grant'), ['SU', 'customer_cycles_create']))==1)
                <a href="#"><i id="create_record" class="bi bi-plus fa-1x"></i></a>
                @endif
                <form class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search" action="{{ url('/customers/cycles') }}" method="get">
    ...
    <div class="card-body">
      <div class="table-responsive">
        <table class="table table-hover" id="dataTable" width="100%" cellspacing="0">
          <thead>
            <tr>
               <th>{{__('customer_cycles.fields.customer_id')}}</th>
               <th>{{__('customer_cycles.fields.activity')}}</th>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="form-group row">
                <label class="col-md-3">{{__('customer_cycles.fields.customer_id')}}</label>
                <div class="col-md-9">
                    <input type="number" name="customer_id" id="customer_id" class="form-control">
                </div>
            </div><div class="form-group row">
                <label class="col-md-3">{{__('customer_cycles.fields.activity')}}</label>
                <div class="col-md-9">
                    <input type="text" name="activity" id="activity" class="form-control">
                </div>
            </div><div class="form-group row">
                <label class="col-md-3">{{__('customer_cycles.fields.time')}}</label>
                <div class="col-md-9">
                    <input type="text" name="time" id="time" class="form-control">
                </div>
            </div><div class="form-group row">
                <label class="col-md-3">{{__('customer_cycles.fields.frequency')}}</label>
                <div class="col-md-9">
